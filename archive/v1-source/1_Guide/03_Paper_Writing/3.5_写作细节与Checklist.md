...
| 语义功能 | 推荐动词 | 示例 |
|---------|---------|------|
| 定义与界定 | define, formalize, characterize, outline | "We formalize Text-to-SQL as a XXX problem." |
| 描述现象 | observe, note, report, identify | "We observe that existing models struggle with schema linking." |
| 提出问题 | highlight, motivate, show, demonstrate, indicate | "This observation motivates a closer look at compositional reasoning." |
| 总结局限 | suffer from, fail to, lack, overlook | "Prior work often fails to handle implicit constraints." |
| 陈述目标 | aim to, seek to, focus on, pursue | "We aim to build a cost-efficient reasoning workflow." |

### 2. Method：描述方案与创新

| 语义功能 | 推荐动词 | 示例 |
|---------|---------|------|
| 提出方案 | introduce, propose, develop, design, present | "We propose a framework that decouples reasoning and execution." |
| 构建与实现 | implement, integrate, compose, extend, propose, introduce, design | "We integrate retrieval and reasoning via adaptive controllers." |
| 改进机制 | enhance, refine, optimize, adjust, augment | "We refine the decoding process with constraint-aware filtering." |
| 建模表示 | model, capture, represent, encode, approximate | "The model captures contextual dependencies through self-attention." |
| 分析设计动机 | motivate, justify, derive | "This design choice is motivated by efficiency–accuracy trade-offs." |

### 3. Experiments：展示与解释结果

| 语义功能 | 推荐动词 | 示例 |
|---------|---------|------|
| 展示结果 | show, demonstrate, present, report, illustrate | "The results demonstrate consistent improvement on Spider." |
| 分析现象 | analyze, compare, inspect, evaluate, show, report, observe, illustrate | "We analyze the effect of prompt length on accuracy." |
| 强调发现 | indicate, suggest, confirm, support, show, demonstrate, highlight, interestingly, surprisingly | "Findings suggest that multi-step reasoning improves stability." |
| 验证有效性 | validate, verify, confirm | "Ablation studies validate the contribution of schema retrieval." |
| 探讨趋势 | observe, notice, identify, highlight | "We observe a diminishing return beyond 7B parameters." |

### 4. Discussion：抽象出洞见与启示

| 语义功能 | 推荐动词 | 示例 |
|---------|---------|------|
| 总结发现 | find, reveal, uncover, discover | "Our findings reveal a critical gap in multi-turn reasoning." |
| 提出洞见 | suggest, imply, point to | "These results suggest that scaling alone is insufficient." |
| 讨论局限 | acknowledge, note, recognize | "We acknowledge that our evaluation is limited to English." |
| 展望方向 | envision, anticipate, call for | "We envision extending this framework to multi-modal settings." |
...
| 段落 | 内容 | 写作要点 |
|---|---|---|
| **第一段** | 本文解决什么重要问题，给出具体应用例子 | 解决什么问题，有什么应用 |
| **第二段** | 为什么需要解决？现有方法的问题是什么？ | 高度凝练现有方法的 Limitations，直击要害，**切记不要罗列太多细节**；新问题说明应用价值；老问题说明现有方法根本缺陷 |
| **第三段** | 有什么 Challenges？为什么难？ | 写**问题的** challenges，不是方法的 challenges；要让人能看懂；challenges 之间逻辑清晰，不要互相覆盖 |
| **第四段** | 如何解决？给出方法总结 | 方法和 challenges 要对应；所有 claim 的优势都要有兑现；**一定要有创新，不要写没有创新的内容** |
| **第五段** | 实验结果 | 超过了现有方法，具体数据 |
| **第六段** | 总结贡献：3 个创新点 + 实验结果 | contributions 和 challenges 一定要对应！ |
